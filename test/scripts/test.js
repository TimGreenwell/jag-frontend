...
import BasicMapStorage from '/scripts/map-storage.js';
...
storages.forEach(
	(storage) => suite(`Test storage ${storage.name}`, async () => {
		await storage.instance.init();
		const service = JAGService.createInstance(`${storage.id}-service`, storage.instance);
